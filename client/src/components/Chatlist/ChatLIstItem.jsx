...
function ChatLIstItem({data, isContactsPage = false}) {
  ...
  const handleContactClick = () => {
    if(!isContactsPage){
      ...
      dispatch({ type: reducerCases.CLEAR_CONTACT_UNREAD, contactId });
    }else{
      dispatch({
      ...
      <div className="min-h-full flex flex-col justify-center mt-3 pr-2 w-full">
        <div className="flex justify-between">
            ...
            className={`text-white ${
              data.totalUnreadMessages > 0 && !isActive ? "font-semibold" : "font-normal"
            }`}
            >
              {data?.name}
            </span>
          </div>
          {
            !isContactsPage && (
              <div>
                <span className={`${data.totalUnreadMessages>0 && !isActive?"text-icon-green":"text-secondary"} text-sm`}>
                  {calculateTime(data.createdAt)}</span>
              </div>
            )
          }
        </div>
        <div className="flex border-b border-conversation-border pb-2 pt-1 p3-2">
          <div className="flex justify-between w-full">
            ...
            className={`line-clamp-1 text-sm ${
              data.totalUnreadMessages > 0 && !isActive
            ...
            {
              data.totalUnreadMessages>0 && !isActive && (<span className="bg-icon-green px-[5px] rounded-full text-sm">{data.totalUnreadMessages}</span>)
            }
          </div>
        </div>
      </div>

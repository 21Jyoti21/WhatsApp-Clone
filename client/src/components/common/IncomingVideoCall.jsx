...
function IncomingVideoCall() {
  ...
  const rejectCall = () => {
    if (userInfo && incomingVideoCall) {
      logCall({
        userId: userInfo.id,
        otherUserId: incomingVideoCall.id,
        otherUserName: incomingVideoCall.name,
        direction: "incoming",
        callType: "video",
        status: "rejected",
        dispatch,
  ...
  return (
    <div className="h-24 w-80 fixed bottom-8 mb-0 right-6 z-50 rounded-sm flex gap-5 items-center justify-start p-4 bg-conversation-panel-background text-white drop-shadow-2xl border-icon-green border-2 py-14">
      <div>
        <Image
          src={incomingVideoCall.profilePicture || "/default_avatar.png"}
          alt="avatar"
          width={70}
          height={70}
